...
/* WEBPACK VAR INJECTION */(function($, jQuery) {__webpack_require__(3);
...
openPanel = function openPanel(content, d) {
    var delay = d || 150;
...
openBigWindow = function openBigWindow(content, delay, callback) {
    delay = delay || 500;

    $('.large-panel-container, .large-panel-exit').remove();

    $('body').prepend('<div class="large-panel-exit"></div>' + '<div class="large-panel-container">' + '<div class="large-panel-inner">' + content + '</div>' + '</div>');
    ...
    $('.large-panel-container').waitForImages(function () {
        ...
        $('.large-panel-container, .large-panel-sidebar').animate({ "top": "0" }, delay, "easeInOutCirc", function () {
...
setUrl = function setUrl(url, title) {
    ...
    $.fn.missionSpinner = function (show) {
        ...
        caller.removeClass('spinner').find('.mission-spinner').remove();

        if (show) {
            caller.addClass('spinner').find('.mission-item-inner').prepend('\
